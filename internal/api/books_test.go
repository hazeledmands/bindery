...
func TestDeleteSiblings(t *testing.T) {
	dir := t.TempDir()
	...
	// Create Book.epub (tracked), Book.mobi, Book.azw3 (siblings), Other.epub (unrelated).
	tracked := write("Book.epub")
	write("Book.mobi")
	write("Book.azw3")
	write("Other.epub")

	// Simulate removeBookPath already having deleted the tracked file.
	if err := os.Remove(tracked); err != nil {
		t.Fatal(err)
	}

	// deleteSiblings should sweep Book.mobi and Book.azw3, leave Other.epub.
	deleteSiblings(tracked)

	for _, name := range []string{"Book.mobi", "Book.azw3"} {
		if _, err := os.Stat(filepath.Join(dir, name)); !os.IsNotExist(err) {
			t.Errorf("expected %s to be deleted, err=%v", name, err)
		}
	}
	if _, err := os.Stat(filepath.Join(dir, "Other.epub")); err != nil {
		t.Errorf("Other.epub should still exist: %v", err)
	}
}

func TestDeleteSiblingsSkipsDirectories(t *testing.T) {
	dir := t.TempDir()
	audiobookDir := filepath.Join(dir, "Audiobook")
	if err := os.Mkdir(audiobookDir, 0o750); err != nil {
		t.Fatal(err)
	}
	// No extension → should be treated as audiobook directory, nothing swept.
	deleteSiblings(audiobookDir)
	if _, err := os.Stat(audiobookDir); err != nil {
		t.Errorf("audiobook directory should not be removed: %v", err)
	}
}
